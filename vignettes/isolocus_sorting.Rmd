...
keepind <- sort(c(which(ploidies == "2x"), 
                  match(c("KMS346", "KMS444", "UI11-00032"), rownames(hh))))
hh <- hh[keepind,]
...
which those samples are, then remove them from the dataset.
...
Again, we expect values below 0.5 ($\frac{ploidy - 1}{ploidy}$).  The tail that
we observe above that value likely represents groups of tags that aligned to the
same location but in fact represent paralogous loci.  The values near zero
likely represent loci with very high amplification bias or overdispersion.  We
will take the peak at 0.3 to represent typical well-behaved markers.  From
this we estimate inbreeding (*F*):
...
directory as `process_isoloci.py`.  We will use the samples list and inbreeding
coefficient that we generated above with R.
...
We can now import the sorted dataset into polyRAD.

```{r eval = FALSE}
myRAD <- readProcessIsoloci("Msa_split_1_sorted.csv")
```
...
```{r}
hist(hh2ByInd, xlab = "Hind/He", main = "Samples", breaks = 50, col = "lightgrey")
