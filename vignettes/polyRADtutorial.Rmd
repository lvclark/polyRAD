...
Markers with values above 0.5 indicate a violation of diploid segregation, but
since we are also modeling allotetraploidy we will leave them in for now.  If
we wanted to remove them, we could identify them from the output of `colMeans`
and remove them using `SubsetByLocus` (see section on mapping populations).
...
The $H_{ind}/H_E$ statistic, to be described in an upcoming preprint, helps to
