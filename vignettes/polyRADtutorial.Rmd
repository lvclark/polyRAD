...
sapply(overdispersionP, quantile, probs = c(0.01, 0.25, 0.5, 0.75, 0.99))
```
...
```{r}
set.seed(720)
ExpectedHindHeMapping(mydata, ploidy = 2, overdispersion = 12, reps = 2)
```
...
```{r}
goodMarkers <- colnames(myhindhe)[which(colMeans(myhindhe, na.rm = TRUE) < 0.52 &
                                          colMeans(myhindhe, na.rm = TRUE) > 0.41)]
...
```{r}
set.seed(803)
ExpectedHindHe(mydata, inbreeding = 0.25, ploidy = 2, overdispersion = 10,
               reps = 10)
```

According to these results, good quality markers can be expected to have
$H_{ind}/H_E$ values from about 0.24 to 0.47.  Values lower than that
...
```{r}
mean(myhindheByLoc < 0.24) # about 33% of markers would be removed
