...
In this case, all of the markers look fine, but if they didn't, we would filter
poor quality markers from the dataset.
...
Before we perform genotype calling, we can test for diploid segregation at
...
argument of `hist`), but let's say the peak is at 0.35.  We can estimate an
inbreeding coefficient from this value:

```{r}
InbreedingFromHindHe(0.35, ploidy = 2)
```

From there, we can simulate what the $H_{ind}/H_E$ distribution might look like
if the dataset consisted entirely of Mendelian diploid loci with mild
overdispersion and no other technical issues.  Here `reps` is set to 10 because
