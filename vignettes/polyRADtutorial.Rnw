...
\texttt{GetWeightedMeanGenotypes}.  If you need discrete numerical genotypes, 
see \texttt{GetProbableGenotypes}.  Also, \texttt{GetLikelyGen} returns the 
...
\begin{itemize}
\item \texttt{SubsetByTaxon}
\item \texttt{SubsetByLocus}
\item \texttt{SplitByChromosome}
\item \texttt{MergeRareHaplotypes}
\item \texttt{RemoveMonomorphicLoci}
\item \texttt{EstimateContaminationRate}
\item \texttt{StripDown}
\end{itemize}

See \texttt{?GetTaxa} for a list of accessor functions as well.
...
\texttt{\$likelyGeno\_donor} and \texttt{\$likelyGeno\_recurrent}.
