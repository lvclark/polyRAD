...
Now we'll add the posterior genotype probabilities.  These are still
...
Now we can export the results for use in downstream analysis.  The function
below selects the best-fit ploidy for each allele based on the results
