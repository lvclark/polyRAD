...
of \emph{Miscanthus sinensis} that was output by the 
...
found in the ``vignettes'' folder of the polyRAD installation.
...
Now we will estimate allele frequencies.  In this case the allele
frequency estimation is mostly so we can get a reasonable estimate of
how often alleles will show up due to contamination.  In a diversity
panel, the allele frequencies can also be used for estimating genotype
frequencies, but here we are going to estimate genotype frequencies
based on the parent genotypes.  Since tetraploidy is possible, we 
will allow allele frequencies to be 0.125, 0.25, 0.375, and so on
(\texttt{expectedFreqs} argument).
We'll specify that we don't want to include the parents or double haploid
lines in the estimation (\texttt{excludeTaxa} argument).  The
...
allele frequency and still be classified as that allele frequencies.  
...
Now we can estimate the likelihood of the data for each possible
...
The next step is to estimate the prior genotype probabilities.  Based on the
likelihoods calculated above, the most likely genotypes for the parents are
assumed to be correct, as long as the ratio of the highest likelihood
to the second highest likelihood is sufficiently high 
(\texttt{minLikelihoodRatio} argument).  The default settings assume an
...
Now we want to get ploidy likelihoods for each locus, \emph{i.e.} the
probability of seeing the observed distribution of genotypes under
the expected genotype frequencies for a given ploidy.  This is performed
using only high-confidence genotypes (again adjustable with 
\texttt{minLikelihoodRatio}).

<<>>=
mydata <- AddPloidyLikelihood(mydata, 
                              excludeTaxa = c("Kaskade-Justin", "Zebrinus-Justin",
                                              "IGR-2011-001", "p196-150A-c", 
                                              "p877-348-b"),
                              minLikelihoodRatio = 100)
mydata$ploidyLikelihood[,1:10]
@
